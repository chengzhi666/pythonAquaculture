...
CREATE TABLE IF NOT EXISTS product_snapshot (
  ...
  INDEX idx_type_spec_time (product_type, spec_weight_normalized, snapshot_time),
...
INSERT INTO product_type_dict(product_type, pattern, keyword_hint, priority, confidence, is_active, note)
VALUES
  ('king_salmon', '(帝王鲑|帝王三文鱼|king\\s*salmon|chinook)', '帝王鲑', 10, 0.9800, 1, '帝王鲑核心词'),
  ('king_salmon', '(帝王鲑|king\\s*salmon)', 'king salmon', 15, 0.9600, 1, '英文关键词'),
  ('rainbow_trout', '(虹鳟|rainbow\\s*trout)', '虹鳟', 20, 0.9800, 1, '虹鳟核心词'),
  ('rainbow_trout', '(虹鳟|rainbow\\s*trout)', 'rainbow', 25, 0.9400, 1, '虹鳟英文简称'),
  ('salmon_generic', '(三文鱼|salmon)', NULL, 90, 0.7000, 1, '通用三文鱼')
ON DUPLICATE KEY UPDATE
  keyword_hint = VALUES(keyword_hint),
  priority = VALUES(priority),
  confidence = VALUES(confidence),
  is_active = VALUES(is_active),
  note = VALUES(note);

INSERT INTO spec_dict(pattern, normalized_unit, gram_factor, priority, is_active, note)
VALUES
  ('^(kg|千克|公斤)$', 'kg', 1000.000000, 10, 1, '千克'),
  ('^(g|克)$', 'g', 1.000000, 20, 1, '克'),
  ('^(斤)$', 'jin', 500.000000, 30, 1, '斤'),
  ('^(两)$', 'liang', 50.000000, 40, 1, '两'),
  ('^(lb|lbs|磅)$', 'lb', 453.592370, 50, 1, '磅'),
  ('^(oz|盎司)$', 'oz', 28.349523, 60, 1, '盎司')
ON DUPLICATE KEY UPDATE
  normalized_unit = VALUES(normalized_unit),
  gram_factor = VALUES(gram_factor),
  priority = VALUES(priority),
  is_active = VALUES(is_active),
  note = VALUES(note);
...
VALUES
  ('智利', '智利', NULL, NULL, '智利', 10, 1, '进口产地'),
  ('挪威', '挪威', NULL, NULL, '挪威', 20, 1, '进口产地'),
  ('法罗', '法罗群岛', NULL, NULL, '法罗群岛', 30, 1, '进口产地'),
  ('青海', '中国', '青海', NULL, '中国-青海', 40, 1, '国内主产地'),
  ('新疆', '中国', '新疆', NULL, '中国-新疆', 50, 1, '国内主产地'),
  ('西藏', '中国', '西藏', NULL, '中国-西藏', 60, 1, '国内冷水产地')
ON DUPLICATE KEY UPDATE
  normalized_country = VALUES(normalized_country),
  normalized_province = VALUES(normalized_province),
  normalized_city = VALUES(normalized_city),
  normalized_origin = VALUES(normalized_origin),
  priority = VALUES(priority),
  is_active = VALUES(is_active),
  note = VALUES(note);
